...
### *Bitta buyruq. 47 ta top AI CLI. Cheksiz imkoniyat.*
...
**Aidevix CLI** — terminaldagi 47 ta top AI CLI agentini (Claude Code, Codex,
...
| 🎛️ | **Yagona `aidevix` menyusi** | 47 ta top AI CLI bitta interaktiv ro'yxatda (status + preview) |
...
| 46 | 🔎 Cody | `cody` | Coding | 🌐 / 🔑 |
| 47 | 🛠 Gorilla CLI | `gorilla` | Chat | 🆓 **bepul** |

> **Login belgilari:** 🔑 API kalit · 🌐 brauzer orqali login · 💳 obuna · 🆓 **bepul** (ochiq manba / bepul tier).
> 💡 **`aidevix --free`** — faqat bepul agentlarni ko'rsatadi (22 ta).
...
> GitHub CLI, Antigravity, Gorilla — 22 ta) ko'rsatadi. `aidevix --top` — eng
> mashhurlarini.
...
└── config/
    └── agents.conf       # Agentlar ro'yxati (47 ta top AI CLI)
```
